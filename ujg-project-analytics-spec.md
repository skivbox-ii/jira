...
## Блок 4.5: Developer Analytics (Аналитика по разработчикам)

### Цель
Детальная аналитика по каждому разработчику, который делал коммиты за период. Показываем только тех, кто реально коммитил код.

### Источники данных
- **Worklogs** (`/rest/api/2/issue/{key}/worklog`) — кто взял задачу в работу
- **Dev-status** (`/rest/dev-status/1.0/issue/detail`) — коммиты, PR, мержи
- **Changelog** (`/rest/api/2/issue/{key}?expand=changelog`) — переходы статусов

### Метрики по разработчику

#### Общая статистика
- **Коммитов**: общее количество коммитов за период
- **Pull Requests**: количество созданных PR
- **Мержей**: количество успешно смерженных PR
- **Задач в работе**: количество задач, где разработчик делал коммиты

#### Детальная цепочка по задаче

Для каждой задачи, где разработчик коммитил:

1. **Взял задачу → первый коммит**
   - Когда взял (первый worklog за период)
   - Когда сделал первый коммит
   - Время между взятием и первым коммитом

2. **Частота коммитов**
   - Количество коммитов на задачу
   - Коммиты каждый день (да/нет)
   - Средний интервал между коммитами

3. **Завершение задачи**
   - Время от последнего коммита до закрытия задачи
   - Задача закрыта и больше не открывалась (стабильно закрыта)
   - Задача вернулась на доработку (после тестирования → work)

4. **Переходы статусов после коммита**
   - После коммита задача ушла в завершающий статус (done)
   - После коммита задача вернулась в work (возврат на доработку)

### UI: Developer Analytics Section
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ФИЛЬТР: Показаны только разработчики, которые делали коммиты за период       │
│                                                                                 │
│  ┌──────────────────────────────────────────────────────────────────────────┐  │
...
│  │    • Взял задачу → первый коммит: 1.2 дня                               │  │
│  │    • Коммитов на задачу: 5.6                                             │  │
│  │    • Последний коммит → закрытие: 2.3 дня                                │  │
│  │                                                                          │  │
│  │ ✅ Качество:                                                             │  │
│  │    • Стабильно закрыто (не открывалось): 7 задач                         │  │
│  │    • Вернулось на доработку: 1 задача                                   │  │
│  │    • После коммита → done: 6 задач                                     │  │
│  │    • После коммита → work (возврат): 1 задача                           │  │
│  │                                                                          │  │
│  │ 📋 Детали по задачам:                                                   │  │
│  │    ┌────────┬──────────────┬──────┬──────────┬──────────┬──────────┐ │  │
│  │    │ Задача │ Взял → Коммит │ Комм │ Комм/день │ Закрыто  │ Возврат  │ │  │
│  │    ├────────┼──────────────┼──────┼──────────┼──────────┼──────────┤ │  │
│  │    │PROJ-89 │    0.5 дня    │  3   │    ✓     │   ✓      │    —     │ │  │
│  │    │PROJ-92 │    1.2 дня    │  7   │    ✓     │   ✓      │    —     │ │  │
│  │    │PROJ-95 │    2.1 дня    │  2   │    —     │   ✓      │    —     │ │  │
...
### Логика определения разработчика

**Разработчик = тот, кто делал коммиты за период**

1. Собираем всех авторов коммитов из `dev-status`
2. Для каждого автора:
   - Находим все задачи, где он коммитил
   - Строим timeline: worklog → коммиты → статусы
   - Считаем метрики

**Если человек не коммитил за период** → не показываем в секции

### Детализация метрик

#### Взял задачу → первый коммит
```javascript
{
    issueKey: "PROJ-123",
    firstWorklog: "2024-01-15T10:00:00",  // Первый worklog за период
    firstCommit: "2024-01-16T14:30:00",   // Первый коммит
    daysToFirstCommit: 1.19                // Разница в днях
}
```

#### Коммиты каждый день
- Если на задаче было >1 коммита
- Проверяем: все ли коммиты в разные дни
- Если коммиты в один день → не считается "каждый день"

#### Стабильно закрыто
- Задача перешла в статус категории "done"
- После этого НЕ было возврата из "done" в другие статусы
- В changelog нет переходов из "done" → другие статусы

#### Вернулось на доработку
- После коммита задача перешла в статус категории "testing"
- Затем вернулась в статус категории "work"
- Считаем количество таких переходов

### Задачи блока
- [x] 4.5.1 Сбор всех разработчиков из коммитов (dev-status)
- [x] 4.5.2 Построение timeline: worklog → коммиты → статусы для каждой задачи
- [x] 4.5.3 Расчёт метрик: взял → коммит, частота коммитов, завершение
- [x] 4.5.4 Детекция стабильно закрытых задач
- [x] 4.5.5 Детекция возвратов на доработку
- [x] 4.5.6 UI секция с детальной статистикой по каждому разработчику
- [x] 4.5.7 Таблица детализации по задачам разработчика
